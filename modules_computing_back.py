...
def angle_cal(psf, dcd, output):
    outfilelis = ['%s/coods_resid4' % (output), '%s/coods_resid2' % (output)]
    if not (os.path.isfile(outfilelis[0]) and os.path.isfile(outfilelis[1])):

        flag = True if "glycine" in os.path.abspath(
            psf) or "gly_" in os.path.abspath(psf) else False
        # will be calculating pseudo dihedral angles + actual dihedral angles for 2 and 4
        atom = "HA1" if flag else "CB"
        string2 = '''
        mol load psf %s dcd %s
        set outdatafile2 [open "%s/coods_resid2" w]
        set outdatafile4 [open "%s/coods_resid4" w]
        puts $outdatafile2 "frame\tresid2CAxyz\tresid2CBxyz"
        puts $outdatafile4 "frame\tresid4CAxyz\tresid4CBxyz"

        set g2_1 [atomselect top "resid 2 and name CA and chain C"]
        set g2_2 [atomselect top "resid 2 and name %s and chain C"]
        set g4_1 [atomselect top "resid 4 and name CA and chain C"]
        set g4_2 [atomselect top "resid 2 and name %s and chain C"]
        ...
        $g4_2 frame $i
        $g4_2 update
        set a1 [$g2_1 get {x y z}]
        set a2 [$g2_2 get {x y z}]
        set b1 [$g4_1 get {x y z}]
        set b2 [$g4_2 get {x y z}]

        puts $outdatafile2 "$i\t$a1\t$a2"
        ...
        close $outdatafile4
        exit
        ''' % (psf, dcd, output, output, atom, atom)
...
def peakdistances(datastream, lis, needmaxorlow):
    upper, lower = lis
    # datastream will be 2d array, first one displacemnet and second one force
